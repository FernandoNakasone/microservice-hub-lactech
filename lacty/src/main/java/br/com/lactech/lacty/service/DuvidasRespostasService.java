...
public class DuvidasRespostasService {
    ...
    public DuvidasRespostasResponseDTO processarPergunta(DuvidasRespostasRequestDTO request) {
        ...
        String duvidaTratada = pergunta.length() > 250
                ? resumirTexto(
                pergunta,
                "Resuma a dúvida do usuário em no máximo 250 caracteres, mantendo a essência técnica."
        )
                : pergunta;
        ...
        if (cpf != null) {
            ...
            if (usuarioCadastrado) {
                ...
                String cepSalvo = usuarioOpt.isPresent()
                        ? usuarioOpt.get().getCep()
                        : "";

                BancoLeiteDTO bancoProximo = buscarBancoPorCep(cepSalvo);

                String infoBanco = bancoProximo != null
                        ? " O ponto de coleta Lactare mais próximo é o "
                          + bancoProximo.getNome()
                          + " (Endereço: "
                          + bancoProximo.getEndereco()
                          + " | Contato: "
                          + bancoProximo.getTelefone()
                          + ")."
                        : "";

                contextoUsuario =
                        " [SISTEMA: O usuário confirmou o CPF "
                                + cpf
                                + " e ELE JÁ ESTÁ CADASTRADO com o CEP "
                                + cepSalvo
                                + "."
                                + infoBanco
                                + " Cumprimente-o de volta, reforce o ponto de coleta e pergunte qual é a dúvida dele sobre leite humano.] ";

            } else {

                contextoUsuario =
                        " [SISTEMA: O usuário informou o CPF "
                                + cpf
                                + " mas ELE NÃO ESTÁ CADASTRADO. "
                                + "Inicie a entrevista para coletar: Idade, CEP e Número. "
                                + "Faça UMA pergunta por vez de forma humanizada.] ";
            }
        }

        String promptSistema =
                "Você é a Lacty, assistente virtual da lactech, um projeto que visa aumentar o engajamento sobre o tema de doação de leite humano."
        ...
        if (resposta != null && resposta.contains("[CADASTRAR:")) {
            ...
            try {
                ...
                novoUsuario.setIdade(
                        Integer.parseInt(partes[1].trim())
                );

                novoUsuario.setCep(cepCadastrado);

                novoUsuario.setNumero(
                        partes[3].trim()
                );
                ...
                BancoLeiteDTO bancoProximo =
                        buscarBancoPerCepLocal(cepCadastrado);
                ...
                if (bancoProximo != null) {
                    ...
                    infoBancoMsg =
                            " Seu ponto de coleta Lactare mais próximo é o "
                                    + bancoProximo.getNome()
                                    + " ("
                                    + bancoProximo.getTelefone()
                                    + ").";
                ...
                String tagCompleta =
                        resposta.substring(
                                resposta.indexOf("[CADASTRAR:"),
                                fim + 1
                        );

                resposta =
                        resposta.replace(tagCompleta, "").trim()
                                + infoBancoMsg;

            } catch (Exception e) {

                System.out.println(
                        "Erro ao tentar fazer o parser do cadastro via IA: "
                                + e.getMessage()
                );
            }
            ...
            if (resposta.contains("Não respondo esse tipo de pergunta")
                    || resposta.contains("Como posso te ajudar?")
                    || resposta.contains("até logo")) {
            ...
            try {
                ...
                if (perguntaRelacionada) {
                    ...
                    DuvidasRespostas novoRegistro =
                            new DuvidasRespostas();

                    novoRegistro.setDuvida(duvidaTratada);
                    novoRegistro.setResposta(resposta);
                    novoRegistro.setData(LocalDateTime.now());

                    DuvidasRespostas registroSalvo =
                            duvidasRespostasRepository.save(novoRegistro);

                    PerformanceChatbotRequestDTO interacao =
                            new PerformanceChatbotRequestDTO(
                                    cpfUsuario,
                                    registroSalvo.getId()
                            );

                    performanceChatbotClient
                            .salvarInteracao(interacao);

                    return new DuvidasRespostasResponseDTO(
                            registroSalvo
                    );
                }
        ...
        DuvidasRespostas respostaNaoSalva =
                new DuvidasRespostas();

        respostaNaoSalva.setDuvida(duvidaTratada);
        respostaNaoSalva.setResposta(resposta);
        respostaNaoSalva.setData(LocalDateTime.now());
    ...
    public List<DuvidasRespostasResponseDTO> findAllPerguntasRespostas(){

        return duvidasRespostasRepository.findAll().stream().map(DuvidasRespostasResponseDTO :: new).toList();
    ...
    private BancoLeiteDTO buscarBancoPorCep(
            String cepUsuario
    ) {
    ...
    private BancoLeiteDTO buscarBancoPerCepLocal(
            String cepUsuario
    ) {

        if (cepUsuario == null || cepUsuario.isEmpty()) {
            return null;
        }
        ...
        try {
            ...
            List<BancoLeiteDTO> bancos =
                    mapper.readValue(
                            is,
                            new TypeReference<List<BancoLeiteDTO>>() {
                            }
                    );
